defmodule Elmc.Backend.Plan.Lower.Case.CharSwitch do
  @moduledoc false
    ...
    tagged? and
      Enum.all?(branches, fn branch ->
        case Map.get(branch, :pattern) do
          %{kind: :wildcard} -> true
          %{kind: :char, value: value} when is_integer(value) -> true
          _ -> false
        end
      end)
  end
  ...
  def compile(subject, branches, ctx, b) do
    with {:ok, subj_reg, b1} <- Expr.compile(subject, ctx, b),
         {:ok, code_reg, b2} <- Expr.compile_runtime_builtin(:char_to_code, [subj_reg], ctx, b1) do
      compile_cfg(code_reg, branches, ctx, b2)
    else
      _ -> :unsupported
    end
  end
  ...
  @spec split_branches(Types.case_branches()) :: {Types.case_branches(), map() | nil}
  defp split_branches(branches) do
    tagged = Enum.filter(branches, fn br -> match?(%{pattern: %{kind: :char}}, br) end)
  ...
  defp compile_arm_blocks(tagged, default_br, ctx, b, merge_reg) do
    with {:ok, tagged_results, arm_exits, b1} <-
           compile_tagged_arms(tagged, ctx, b, merge_reg, []),
  ...
  @spec compile_tagged_arms(
          ...
          {:ok, [tagged_arm()], [non_neg_integer()], Builder.t()} | :unsupported
  ...
  defp compile_tagged_arms([branch | rest], ctx, b, merge_reg, acc) do
    ...
    with {:ok, reg, tag, b1} <- compile_one_arm(branch, ctx, b_arm),
         ...
         {:ok, more, exits, b3} <-
           compile_tagged_arms(rest, ctx, b2, merge_reg, [{tag, reg, arm_id} | acc]) do
  ...
  @spec compile_one_arm(map(), Context.t(), Builder.t()) ::
          {:ok, Types.reg(), integer() | nil, Builder.t()} | :unsupported
  defp compile_one_arm(branch, ctx, b) do
    ...
    case pattern do
      ...
      %{kind: :char, value: value} when is_integer(value) ->
        case Expr.compile(expr, branch_ctx, b) do
