defmodule Elmc.Backend.Plan.Lower.SpecialValues.Draw do
  @moduledoc false
  ...
  defp ui_label_expr(%{op: op, target: target} = expr)
       when op in [:constructor_call, :qualified_call, :constructor_ref, :qualified_ref, :qualified_var] and
              is_binary(target) do
    case ui_label_text(target) do
      text when is_binary(text) -> %{op: :string_literal, value: text}
      _ -> expr
    end
  end

  defp ui_label_expr(%{op: :int_literal, union_ctor: ctor} = expr) when is_binary(ctor) do
    case ui_label_text(ctor) do
      text when is_binary(text) -> %{op: :string_literal, value: text}
      _ -> expr
    end
  end
