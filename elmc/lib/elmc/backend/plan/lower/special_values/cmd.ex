defmodule Elmc.Backend.Plan.Lower.SpecialValues.Cmd do
  @moduledoc false
  alias Elmc.Backend.CCodegen.Types, as: Types


  alias Elmc.Backend.Plan.Lower.SpecialValues.Helpers
  alias Elmc.Backend.Plan.Lower.SpecialValues.RandomGenerateFold
  alias Elmc.Backend.Plan.Lower.SpecialValues.Stdlib.Effects
  ...
  def special_value_from_target("Random.generate", [to_msg, generator]) do
    if PlatformWeb.web_target?(Process.get(:elmc_codegen_opts, %{})) do
      nil
    else
      random_generate_cmd_expr(to_msg, generator)
    end
  end

  def special_value_from_target("Elm.Kernel.Random.generate", [to_msg, generator]) do
    if PlatformWeb.web_target?(Process.get(:elmc_codegen_opts, %{})) do
      nil
    else
      random_generate_cmd_expr(to_msg, generator)
  ...
  defp random_generate_cmd_expr(to_msg, generator) do
    tag = Helpers.constructor_tag_expr(to_msg)

    case RandomGenerateFold.fold_int_bounds(generator, %{}) do
      {:ok, lo, hi} ->
        Helpers.encoded_cmd_expr(
          Helpers.command_kind(:random_generate),
          [tag, %{op: :int_literal, value: lo}, %{op: :int_literal, value: hi}],
          3
        )
      ...
      :error ->
        UnsupportedSurface.unsupported_expr(%{
          ...
          arity: 2,
          detail: "Random.generate generator not foldable"
        })
    end
  end
end
