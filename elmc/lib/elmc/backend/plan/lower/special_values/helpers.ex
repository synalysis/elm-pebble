defmodule Elmc.Backend.Plan.Lower.SpecialValues.Helpers do
  @moduledoc false
  ...
  def encoded_cmd_expr(kind, args, arity) do
    if length(args) == arity do
      ...
      UnsupportedSurface.unsupported_expr(%{
        ...
        target: "encoded_cmd",
        arity: arity,
        detail: "expected #{arity} args, got #{length(args)}"
      })
    end
  end
  ...
  def encoded_draw_line_cmd_expr(kind, start_pos, end_pos, color) do
    with {:ok, {sx, sy}} <- point_coord_exprs(start_pos),
      ...
      _ -> unsupported_draw_expr("encoded_draw_line_cmd", 5, "line endpoints not foldable")
    end
  end
  ...
  def text_options_update_expr(_options, _field, _value),
    do:
      UnsupportedSurface.unsupported_expr(%{
        kind: :expr,
        target: "text_options_update",
        detail: "unsupported text options update shape"
      })

  defp unsupported_draw_expr(target, arity, detail) do
    UnsupportedSurface.unsupported_expr(%{
      kind: :cmd,
      target: target,
      arity: arity,
