defmodule Elmc.Backend.CCodegen.Subscriptions do
  ...
  defp subscription_item_c_expr(%{op: :qualified_call, target: target}) when is_binary(target) do
    case SpecialValues.normalize_special_target(target) do
      ...
      "Pebble.Light.onChange" -> "ELMC_SUBSCRIPTION_BACKLIGHT"
      "Elm.Kernel.PebbleWatch.onBacklightChange" -> "ELMC_SUBSCRIPTION_BACKLIGHT"
      "Pebble.Platform.onScreenChange" -> "ELMC_SUBSCRIPTION_SCREEN_CHANGE"
      "Elm.Kernel.PebbleWatch.onScreenChange" -> "ELMC_SUBSCRIPTION_SCREEN_CHANGE"
      "Pebble.Speaker.onFinished" -> "ELMC_SUBSCRIPTION_SPEAKER_FINISHED"
      "Elm.Kernel.PebbleWatch.onSpeakerFinished" -> "ELMC_SUBSCRIPTION_SPEAKER_FINISHED"
  ...
  @type subscription_mask_analysis :: %{
          tag_masks: [String.t()],
          button_raw_count: non_neg_integer(),
          compact: boolean(),
          dynamic?: boolean(),
  ...
  defp collect_subscription_specs(%{op: :qualified_call, target: target, args: args}, acc)
    ...
    cond do
      ...
      true ->
        collect_single_subscription_specs(normalized, args, acc)
    end
  end
