defmodule Elmc.WasmWebBackendTaskHttpPostTest do
  ...
  @tag :wasm_execute
  test "BackendTask.Http.post with jsonBody and expectWhatever performs browser fetch" do
    cond do
      not execution_tools_available?() ->
        :ok

      true ->
        root = Path.expand("fixtures/wasm_web_backend_task_http_post_project", __DIR__)
        out_dir = Path.expand("tmp/wasm_web_backend_task_http_post", __DIR__)
        File.rm_rf!(out_dir)
        ...
        assert wat =~ "backend_task_http_post"
        assert wat =~ "backend_task_http_json_body"
        assert wat =~ "backend_task_http_expect_whatever"
        ...
        case run_probe(out_dir) do
          ...
          {:error, output} ->
            if probe_skipped_under_ulimit?(output) do
              ...
              flunk("backend task http post probe failed:\n#{output}")
            end
        end
    end
  end

  @runner Path.expand("support/wasm_backend_task_http_post_probe_runner.mjs", __DIR__)
