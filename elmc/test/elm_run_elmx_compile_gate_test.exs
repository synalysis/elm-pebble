defmodule Elmc.ElmRunCorpusElmxCompileGateTest do
  use ExUnit.Case

  alias Elmc.Test.ElmRunCorpus

  @baseline_path Path.expand("../docs/elm_run_corpus_elmx_baseline.json", __DIR__)
  @scorecard_dir Path.expand("tmp/elm_run_corpus_elmx", __DIR__)

  @tag :corpus_elmx_smoke
  @tag timeout: 120_000
  test "smoke elmx compile gate on corpus canary programs" do
    if corpus_skip?() or not ElmRunCorpus.available?() do
      IO.puts("Skipping corpus elmx smoke compile gate")
      :ok
    else
      index = ElmRunCorpus.build_index()
      paths = ElmRunCorpus.elmx_smoke_tests()

      scorecard =
        ElmRunCorpus.run_elmx_compile_gate!(
          index: index,
      ...
      assert failures == [],
             "corpus elmx smoke compile failures: #{inspect(failures, limit: 20)}"
    end
  end

  @tag :corpus_elmx
  @tag timeout: :infinity
  test "full elmx compile gate across portable corpus programs" do
    if corpus_skip?() or not ElmRunCorpus.available?() do
      IO.puts("Skipping corpus elmx compile gate")
      :ok
    else
      index = ElmRunCorpus.build_index()
  ...
  defp assert_scorecard_against_baseline!(scorecard) do
    ...
    assert summary["compile_ok"] >= baseline["compile_ok_min"],
           """
           corpus elmx compile regression:
    ...
    if baseline["corpus_sha"] && scorecard["corpus_sha"] != baseline["corpus_sha"] do
      IO.warn(
        "corpus SHA changed (#{baseline["corpus_sha"]} -> #{scorecard["corpus_sha"]}); refresh elmx baseline after intentional corpus update"
      )
    end
  end
