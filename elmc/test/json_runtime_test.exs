defmodule Elmc.JsonRuntimeTest do
  ...
  defp json_runtime_harness_source do
    ...
    static ElmcValue *json_take_decode_int_decoder(void) {
      ElmcValue *out = NULL;
    ...
    static ElmcValue *json_take_decode_value_decoder(void) {
      ElmcValue *out = NULL;
      return elmc_json_decode_value_decoder(&out) == RC_SUCCESS ? out : NULL;
    }

    static ElmcValue *json_take_decode_field(ElmcValue *name, ElmcValue *decoder) {
      ElmcValue *out = NULL;
      return elmc_json_decode_field(&out, name, decoder) == RC_SUCCESS ? out : NULL;
    }

    static ElmcValue *json_take_decode_string(ElmcValue *decoder, ElmcValue *input) {
      ElmcValue *out = NULL;
      return elmc_json_decode_string(&out, decoder, input) == RC_SUCCESS ? out : NULL;
    }

    static ElmcValue *json_take_decode_index(ElmcValue *idx, ElmcValue *decoder) {
      ElmcValue *out = NULL;
    ...
    static ElmcValue *json_take_decode_map(ElmcValue *f, ElmcValue *decoder) {
      ElmcValue *out = NULL;
      return elmc_json_decode_map(&out, f, decoder) == RC_SUCCESS ? out : NULL;
    }

    static ElmcValue *json_take_decode_map2(ElmcValue *f, ElmcValue *d1, ElmcValue *d2) {
      ElmcValue *out = NULL;
      return elmc_json_decode_map2(&out, f, d1, d2) == RC_SUCCESS ? out : NULL;
    }

    static ElmcValue *json_take_decode_map3(ElmcValue *f, ElmcValue *d1, ElmcValue *d2, ElmcValue *d3) {
      ElmcValue *out = NULL;
      return elmc_json_decode_map3(&out, f, d1, d2, d3) == RC_SUCCESS ? out : NULL;
    }

    static ElmcValue *json_take_decode_map6(ElmcValue *f, ElmcValue *d1, ElmcValue *d2, ElmcValue *d3, ElmcValue *d4, ElmcValue *d5, ElmcValue *d6) {
      ElmcValue *out = NULL;
      return elmc_json_decode_map6(&out, f, d1, d2, d3, d4, d5, d6) == RC_SUCCESS ? out : NULL;
    }

    static ElmcValue *json_take_decode_map7(ElmcValue *f, ElmcValue *d1, ElmcValue *d2, ElmcValue *d3, ElmcValue *d4, ElmcValue *d5, ElmcValue *d6, ElmcValue *d7) {
      ElmcValue *out = NULL;
      return elmc_json_decode_map7(&out, f, d1, d2, d3, d4, d5, d6, d7) == RC_SUCCESS ? out : NULL;
    }

    static ElmcValue *json_take_decode_and_then(ElmcValue *f, ElmcValue *decoder) {
      ElmcValue *out = NULL;
      return elmc_json_decode_and_then(&out, f, decoder) == RC_SUCCESS ? out : NULL;
    }

    static ElmcValue *json_take_decode_key_value_pairs(ElmcValue *decoder) {
      ElmcValue *out = NULL;
    ...
    static ElmcValue *json_take_encode_string(ElmcValue *s) {
      ElmcValue *out = NULL;
    ...
    static ElmcValue *inc(ElmcValue **args, int argc, ElmcValue **captures, int capture_count) {
      (void)captures;
      (void)capture_count;
      return elmc_harness_new_int(argc > 0 ? elmc_as_int(args[0]) + 1 : 0);
    }

    static ElmcValue *sum2(ElmcValue **args, int argc, ElmcValue **captures, int capture_count) {
      (void)captures;
      (void)capture_count;
      if (argc < 2) return elmc_harness_new_int(0);
      return elmc_harness_new_int(elmc_as_int(args[0]) + elmc_as_int(args[1]));
    }

    static ElmcValue *sum3(ElmcValue **args, int argc, ElmcValue **captures, int capture_count) {
      (void)captures;
      (void)capture_count;
      if (argc < 3) return elmc_harness_new_int(0);
      return elmc_harness_new_int(elmc_as_int(args[0]) + elmc_as_int(args[1]) + elmc_as_int(args[2]));
    }

    static ElmcValue *sum6(ElmcValue **args, int argc, ElmcValue **captures, int capture_count) {
      (void)captures;
      (void)capture_count;
      if (argc < 6) return elmc_harness_new_int(0);
      return elmc_harness_new_int(elmc_as_int(args[0]) + elmc_as_int(args[1]) + elmc_as_int(args[2]) +
    ...
    static ElmcValue *sum7(ElmcValue **args, int argc, ElmcValue **captures, int capture_count) {
      (void)captures;
      (void)capture_count;
      if (argc < 7) return elmc_harness_new_int(0);
      return elmc_harness_new_int(elmc_as_int(args[0]) + elmc_as_int(args[1]) + elmc_as_int(args[2]) +
                          elmc_as_int(args[3]) + elmc_as_int(args[4]) + elmc_as_int(args[5]) +
                          elmc_as_int(args[6]));
    }
    ...
    static ElmcValue *encode_int_value(ElmcValue **args, int argc, ElmcValue **captures, int capture_count) {
      (void)captures;
      (void)capture_count;
      if (argc < 1 || !args[0]) return elmc_harness_new_string("0");
      return json_take_encode_int(args[0]);
    }
    ...
    static ElmcValue *name_decoder(ElmcValue **args, int argc, ElmcValue **captures, int capture_count) {
      ...
      (void)captures;
      (void)capture_count;
      ElmcValue *field = elmc_harness_new_string("name");
      ElmcValue *string_decoder = json_take_decode_string_decoder();
      ElmcValue *decoder = json_take_decode_field(field, string_decoder);
      elmc_release(field);
    ...
    static ElmcValue *json_text(const char *text) {
      return elmc_harness_new_string(text);
    }

    static ElmcValue *decode(ElmcValue *decoder, const char *raw) {
      ElmcValue *input = json_text(raw);
      ElmcValue *result = json_take_decode_string(decoder, input);
      elmc_release(input);
      return result;
    }

    int main(void) {
      ElmcValue *screen = elmc_harness_new_string("screen");
      ElmcValue *width = elmc_harness_new_string("width");
      ElmcValue *int_decoder = json_take_decode_int_decoder();
      ElmcValue *width_decoder = json_take_decode_field(width, int_decoder);
      ElmcValue *screen_width_decoder = json_take_decode_field(screen, width_decoder);
      ...
      ElmcValue *tag = elmc_harness_new_string("message_tag");
      int_decoder = json_take_decode_int_decoder();
      ElmcValue *tag_decoder = json_take_decode_field(tag, int_decoder);
      ...
      ElmcValue *idx = elmc_harness_new_int(1);
      int_decoder = json_take_decode_int_decoder();
      ElmcValue *index_decoder = json_take_decode_index(idx, int_decoder);
      ...
      ElmcValue *missing = elmc_harness_new_string("missing");
      int_decoder = json_take_decode_int_decoder();
      ElmcValue *missing_decoder = json_take_decode_field(missing, int_decoder);
      ElmcValue *maybe_decoder = json_take_decode_maybe(missing_decoder);
      ...
      int_decoder = json_take_decode_int_decoder();
      ElmcValue *string_decoder = json_take_decode_string_decoder();
      ElmcValue *decoders = elmc_harness_list_cons(string_decoder, elmc_list_nil());
      decoders = elmc_harness_list_cons(int_decoder, decoders);
      ElmcValue *one_of = json_take_decode_one_of(decoders);
      ...
      int_decoder = json_take_decode_int_decoder();
      ElmcValue *inc_closure = elmc_harness_closure_new(inc, 1);
      ElmcValue *map_decoder = json_take_decode_map(inc_closure, int_decoder);
      ...
      ElmcValue *field_a = elmc_harness_new_string("a");
      ElmcValue *field_b = elmc_harness_new_string("b");
      int_decoder = json_take_decode_int_decoder();
      ElmcValue *a_decoder = json_take_decode_field(field_a, int_decoder);
      ElmcValue *b_decoder = json_take_decode_field(field_b, int_decoder);
      ElmcValue *sum_closure = elmc_harness_closure_new(sum2, 2);
      ElmcValue *map2_decoder = json_take_decode_map2(sum_closure, a_decoder, b_decoder);
      ...
      field_a = elmc_harness_new_string("a");
      field_b = elmc_harness_new_string("b");
      ElmcValue *field_c = elmc_harness_new_string("c");
      int_decoder = json_take_decode_int_decoder();
      a_decoder = json_take_decode_field(field_a, int_decoder);
      b_decoder = json_take_decode_field(field_b, int_decoder);
      ElmcValue *c_decoder = json_take_decode_field(field_c, int_decoder);
      ElmcValue *sum3_closure = elmc_harness_closure_new(sum3, 3);
      ElmcValue *map3_decoder = json_take_decode_map3(sum3_closure, a_decoder, b_decoder, c_decoder);
      ...
      ElmcValue *kind = elmc_harness_new_string("kind");
      int_decoder = json_take_decode_int_decoder();
      ElmcValue *kind_decoder = json_take_decode_field(kind, int_decoder);
      ElmcValue *name_closure = elmc_harness_closure_new(name_decoder, 0);
      ElmcValue *and_then_decoder = json_take_decode_and_then(name_closure, kind_decoder);
      ...
      ElmcValue *payload = elmc_harness_new_string("payload");
      ElmcValue *value_decoder = json_take_decode_value_decoder();
      ElmcValue *payload_decoder = json_take_decode_field(payload, value_decoder);
      ...
      ElmcValue *raw = elmc_harness_new_string("{\\"ok\\":true}");
      ElmcValue *encoded = json_take_encode_string(elmc_harness_new_string("a\\\\nb"));
      if (!encoded || encoded->tag != ELMC_TAG_STRING || strcmp((const char *)encoded->payload, "\\"a\\\\\\\\nb\\"") != 0) return 13;
      elmc_release(encoded);

      ElmcValue *key = elmc_harness_new_string("nested");
      ElmcValue *pair = elmc_harness_tuple2_take(key, raw);
      ElmcValue *pairs = elmc_harness_list_cons(pair, elmc_list_nil());
      ElmcValue *object = json_take_encode_object(pairs);
      ...
      field_a = elmc_harness_new_string("a");
      field_b = elmc_harness_new_string("b");
      field_c = elmc_harness_new_string("c");
      field_d = elmc_harness_new_string("d");
      field_e = elmc_harness_new_string("e");
      field_f = elmc_harness_new_string("f");
      int_decoder = json_take_decode_int_decoder();
      a_decoder = json_take_decode_field(field_a, int_decoder);
      b_decoder = json_take_decode_field(field_b, int_decoder);
      c_decoder = json_take_decode_field(field_c, int_decoder);
      d_decoder = json_take_decode_field(field_d, int_decoder);
      e_decoder = json_take_decode_field(field_e, int_decoder);
      f_decoder = json_take_decode_field(field_f, int_decoder);
      ElmcValue *sum6_closure = elmc_harness_closure_new(sum6, 6);
      ElmcValue *map6_decoder = json_take_decode_map6(sum6_closure, a_decoder, b_decoder, c_decoder, d_decoder, e_decoder, f_decoder);
      ...
      int_decoder = json_take_decode_int_decoder();
      ElmcValue *kvp_decoder = json_take_decode_key_value_pairs(int_decoder);
      ...
      {
        ElmcValue *fa = elmc_harness_new_string("a");
        ElmcValue *fb = elmc_harness_new_string("b");
        ElmcValue *fc = elmc_harness_new_string("c");
        ElmcValue *fd = elmc_harness_new_string("d");
        ElmcValue *fe = elmc_harness_new_string("e");
        ElmcValue *ff = elmc_harness_new_string("f");
        ElmcValue *fg = elmc_harness_new_string("g");
        ElmcValue *int_dec = json_take_decode_int_decoder();
        ElmcValue *da = json_take_decode_field(fa, int_dec);
        ElmcValue *db = json_take_decode_field(fb, int_dec);
        ElmcValue *dc = json_take_decode_field(fc, int_dec);
        ElmcValue *dd = json_take_decode_field(fd, int_dec);
        ElmcValue *de = json_take_decode_field(fe, int_dec);
        ElmcValue *df = json_take_decode_field(ff, int_dec);
        ElmcValue *dg = json_take_decode_field(fg, int_dec);
        ElmcValue *sum7_closure = elmc_harness_closure_new(sum7, 7);
        ElmcValue *map7_decoder = json_take_decode_map7(sum7_closure, da, db, dc, dd, de, df, dg);
      ...
      ElmcValue *dict_key = elmc_harness_new_string("x");
      ElmcValue *dict_val = elmc_harness_new_int(9);
      ElmcValue *dict_pair = elmc_harness_tuple2_take(dict_key, dict_val);
      ElmcValue *dict_list = elmc_harness_list_cons(dict_pair, elmc_list_nil());
      ElmcValue *id_closure = elmc_harness_closure_new(identity, 1);
      ElmcValue *encode_int_closure = elmc_harness_closure_new(encode_int_value, 1);
      ElmcValue *encoded_dict = json_take_encode_dict(id_closure, encode_int_closure, dict_list);
