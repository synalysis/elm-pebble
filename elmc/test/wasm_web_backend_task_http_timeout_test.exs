defmodule Elmc.WasmWebBackendTaskHttpTimeoutTest do
  ...
  @tag :wasm_execute
  test "BackendTask.Http timeoutInMs yields typed Timeout error in browser" do
    cond do
      not execution_tools_available?() ->
        :ok

      true ->
        root = Path.expand("fixtures/wasm_web_backend_task_http_timeout_project", __DIR__)
        out_dir = Path.expand("tmp/wasm_web_backend_task_http_timeout", __DIR__)
        File.rm_rf!(out_dir)
        ...
        assert manifest["constructor_tags"]["BackendTask.Http.Timeout"] != nil
        ...
        case run_probe(out_dir) do
          ...
          {:error, output} ->
            if probe_skipped_under_ulimit?(output) do
              ...
              flunk("backend task http timeout probe failed:\n#{output}")
            end
        end
    end
  end

  @runner Path.expand("support/wasm_backend_task_http_timeout_probe_runner.mjs", __DIR__)
