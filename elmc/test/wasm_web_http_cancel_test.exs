defmodule Elmc.WasmWebHttpCancelTest do
  ...
  @tag :wasm_execute
  test "Http.cancel lowers to http_cancel runtime import" do
    cond do
      not execution_tools_available?() ->
        :ok

      true ->
        root = Path.expand("fixtures/wasm_web_http_cancel_project", __DIR__)
        out_dir = Path.expand("tmp/wasm_web_http_cancel", __DIR__)
        File.rm_rf!(out_dir)
        ...
        case run_probe(out_dir) do
          ...
          {:error, output} ->
            if probe_skipped_under_ulimit?(output) do
              ...
              flunk("http cancel probe failed:\n#{output}")
            end
        end
    end
  end

  @runner Path.expand("support/wasm_http_cancel_probe_runner.mjs", __DIR__)
