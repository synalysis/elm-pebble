defmodule Elmc.QualifiedBuiltinCodegenTest do
  ...
  test "Pebble.Ui.Rect literals in Main use canonical x y w h codegen order" do
    ...
    assert body =~ ~r/elmc_int_t rec_values_\d+_\d+\[4\] = \{ plan_native_int_\d+ - 9, plan_native_int_\d+ - 8, 18, 12 \};/,
  ...
  test "typed Color and String arguments use native direct command parameters" do
    ...
    main_source =
      ...
      |> Kernel.<>(native_text_arg_source())

    out_dir = compile_snippet!("native_text_arg_project", main_source, %{strip_dead_code: false})
  ...
  test "rotationFromDegrees literal folds to Rotation union for path rotation args" do
    ...
    assert bitmap_body =~
             "scene_cmd.p3 = ELMC_RECORD_GET_INDEX_INT(model, ELMC_FIELD_MAIN_MODEL_ROTATIONANGLE)" or
             (bitmap_body =~ "ELMC_RECORD_GET_INDEX_INT(model, ELMC_FIELD_MAIN_MODEL_ROTATIONANGLE)" and
                bitmap_body =~ "scene_cmd.p3 = plan_native_int_")
