defmodule Elmc.WasmWebHttpBehaviorTest do
  ...
  @tag :wasm_execute
  test "Http.post init drains through mock fetch host" do
    cond do
      ...
      true ->
        ...
        out_dir = Path.expand("tmp/wasm_web_http_post", __DIR__)
        File.rm_rf!(out_dir)
        ...
        case run_probe(out_dir) do
          ...
          {:error, output} ->
            if probe_skipped_under_ulimit?(output) do
              ...
              flunk("http behavior probe failed:\n#{output}")
            end
        end
    end
  end

  @runner Path.expand("support/wasm_http_behavior_probe_runner.mjs", __DIR__)
