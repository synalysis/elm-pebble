...
if (!buildDir) {
  console.error("usage: wasm_resize_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
if (before !== "wait") {
  console.error(`expected no resize on subscribe, got ${JSON.stringify(before)}`);
  process.exit(1);
}
...
if (innerText !== "800x600") {
  console.error(`expected official onResize Int Int, got ${JSON.stringify(innerText)}`);
  process.exit(1);
}
