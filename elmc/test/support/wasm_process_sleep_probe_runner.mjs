...
if (!buildDir) {
  console.error("usage: wasm_process_sleep_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
if (!Number.isFinite(millis) || millis <= 0) {
  console.error(
    `expected Process.sleep then Time.now to update the view, got ${JSON.stringify(innerText)}`
  );
  process.exit(1);
}
...
if (!helpers.checkBalanced()) {
  ...
  if (state) {
    console.warn("RC imbalance after process sleep boot (non-fatal)", state);
  }
}

console.log("rc_ok process_sleep_ok");
