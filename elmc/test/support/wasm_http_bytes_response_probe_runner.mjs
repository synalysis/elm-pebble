...
if (!buildDir) {
  console.error("usage: wasm_http_bytes_response_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
globalThis.fetch = async (_url, _init) => {
  fetchCalls += 1;
  const bytes = new Uint8Array([42, 7, 9]);
  return {
    ok: true,
    status: 200,
    statusText: "OK",
...
if (fetchCalls < 1) {
  console.error("expected Http.expectBytesResponse to fetch");
  process.exit(1);
}
const expected = "200:OK:42:3";
if (innerText !== expected) {
  console.error(
    `expected official GoodStatus_ Bytes metadata+body ${JSON.stringify(expected)}, got ${JSON.stringify(innerText)}`
  );
  process.exit(1);
}
...
console.log("rc_ok http_bytes_response_ok");
