...
if (!buildDir) {
  console.error("usage: wasm_task_map_error_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
const expected = "boom!|7";
if (innerText !== expected) {
  console.error(
    `expected official Task.mapError fail rewrite + succeed passthrough ${JSON.stringify(expected)}, got ${JSON.stringify(innerText)}`
  );
  process.exit(1);
}
...
if (!helpers.checkBalanced()) {
  ...
  if (state) {
    console.warn("RC imbalance after task mapError boot (non-fatal)", state);
  }
}

console.log("rc_ok task_map_error_ok");
