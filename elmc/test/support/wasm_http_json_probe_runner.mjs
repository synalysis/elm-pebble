...
if (!buildDir) {
  console.error("usage: wasm_http_json_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
globalThis.fetch = async (_url, _init) => {
  ...
  return {
    ...
    text: async () => JSON.stringify({ n: 7 }),
    arrayBuffer: async () => new TextEncoder().encode(JSON.stringify({ n: 7 })).buffer,
  };
};
...
if (fetchCalls < 1) {
  console.error("expected Http.expectJson to fetch");
  process.exit(1);
}
if (innerText !== "7") {
  console.error(
    `expected Http.expectJson Decode.field n int to show 7, got ${JSON.stringify(innerText)}`
  );
  process.exit(1);
}
...
if (!helpers.checkBalanced()) {
  ...
  if (state) {
    console.warn("RC imbalance after http expectJson boot (non-fatal)", state);
  }
}

console.log("rc_ok http_json_ok");
