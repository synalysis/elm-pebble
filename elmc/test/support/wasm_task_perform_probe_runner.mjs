...
if (!buildDir) {
  console.error("usage: wasm_task_perform_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
if (!Number.isFinite(millis) || millis <= 0) {
  console.error(
    `expected Task.perform Time.now to update the view, got ${JSON.stringify(innerText)}`
  );
  process.exit(1);
}
...
if (!helpers.checkBalanced()) {
  ...
  if (state) {
    console.warn("RC imbalance after task perform boot (non-fatal)", state);
  }
}

console.log("rc_ok task_perform_ok");
