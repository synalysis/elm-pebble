...
if (!buildDir) {
  console.error("usage: wasm_html_style_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
if (!box) {
  console.error(`missing #box html=${document.body?.innerHTML ?? ""}`);
  process.exit(1);
}

const color = String(box.style?.color || "");
const fontSize =
  String(box.style?.fontSize || "") ||
  (typeof box.style?.getPropertyValue === "function"
    ? String(box.style.getPropertyValue("font-size") || "")
    : "");

if (color !== "red" || fontSize !== "12px") {
  console.error(
    `expected official Html.Attributes.style merge color=red font-size=12px, got color=${JSON.stringify(color)} fontSize=${JSON.stringify(fontSize)} style=${JSON.stringify(box.getAttribute("style"))}`
  );
  process.exit(1);
}
...
console.log("rc_ok html_style_ok");
