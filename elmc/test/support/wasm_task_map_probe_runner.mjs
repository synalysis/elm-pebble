...
if (!buildDir) {
  console.error("usage: wasm_task_map_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
if (innerText !== "mapped") {
  console.error(
    `expected Cmd.map to wrap Task.perform Tick, view=${JSON.stringify(innerText)}`
  );
  process.exit(1);
}
...
if (!helpers.checkBalanced()) {
  ...
  if (state) {
    console.warn("RC imbalance after task map boot (non-fatal)", state);
  }
}

console.log("rc_ok task_map_ok");
