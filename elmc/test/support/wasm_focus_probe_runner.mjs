...
if (!buildDir) {
  console.error("usage: wasm_focus_probe_runner.mjs <buildDir>");
  process.exit(2);
}

const { document, window } = parseHTML(
  "<!doctype html><html><body><div id='app'></div><input id='box' /></body></html>"
);
...
if (innerText !== "ok") {
  console.error(
    `expected focus missing→NotFound then focus/blur box, got ${JSON.stringify(innerText)}`
  );
  process.exit(1);
}
if (!focused || !blurred) {
  console.error(`expected host focus then blur on #box, focused=${focused} blurred=${blurred}`);
  process.exit(1);
}
