...
if (!buildDir) {
  console.error("usage: wasm_process_kill_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
const millis = Number.parseInt(innerText, 10);
if (!Number.isFinite(millis) || millis <= 0) {
  console.error(
    `expected Process.kill of a spawned sleep to leave Task.perform sleep running, got ${JSON.stringify(innerText)}`
  );
  process.exit(1);
}
...
if (!helpers.checkBalanced()) {
  ...
  if (state) {
    console.warn("RC imbalance after process kill boot (non-fatal)", state);
  }
}

console.log("rc_ok process_kill_ok");
