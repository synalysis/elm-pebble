...
if (!buildDir) {
  console.error("usage: wasm_process_backend_task_http_kill_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
if (fetchStarted < 1) {
  console.error("expected spawned BackendTask.Http.get to start fetch");
  process.exit(1);
}
if (fetchCompleted > 0) {
  console.error("expected Process.kill to abort spawned BackendTask.Http before completion");
  process.exit(1);
}
if (innerText !== "ok") {
  console.error(
...
console.log("rc_ok process_backend_task_http_kill_ok");
