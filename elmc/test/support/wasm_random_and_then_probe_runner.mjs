...
if (!buildDir) {
  console.error("usage: wasm_random_and_then_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
while (Date.now() < deadline) {
  ...
  if (innerText === "3:a" || innerText === "3:b") break;
  await new Promise((r) => setTimeout(r, 20));
}

if (innerText !== "3:a" && innerText !== "3:b") {
  console.error(
    `expected Random.andThen + pair + uniform 3:a|3:b, got ${JSON.stringify(innerText)}`
  );
  process.exit(1);
}
...
console.log("rc_ok random_and_then_ok");
