...
if (!buildDir) {
  console.error("usage: wasm_animation_frame_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
// linkedom has no rAF. Official AnimationManager uses requestAnimationFrame time.
let rafNow = 16;
...
const requestAnimationFrame = (cb) => {
  ...
  rafIds.set(id, handle);
  return id;
};
const cancelAnimationFrame = (id) => {
  const handle = rafIds.get(id);
...
if (innerText !== "ok") {
  console.error(
    `expected onAnimationFrame Posix + onAnimationFrameDelta Float, got ${JSON.stringify(innerText)}`
  );
  process.exit(1);
}
...
console.log("rc_ok animation_frame_ok");
process.exit(0);
