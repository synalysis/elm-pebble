...
if (!buildDir) {
  console.error("usage: wasm_random_weighted_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
while (Date.now() < deadline) {
  ...
  if (innerText === "9") break;
  await new Promise((r) => setTimeout(r, 20));
}

if (innerText !== "9") {
  console.error(
    `expected Random.independentSeed + weighted (1,9) to yield 9, got ${JSON.stringify(innerText)}`
  );
  process.exit(1);
}
...
console.log("rc_ok random_weighted_ok");
