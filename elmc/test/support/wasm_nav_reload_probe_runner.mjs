...
const [buildDir, expectedForceArg] = process.argv.slice(2);
if (!buildDir) {
  console.error("usage: wasm_nav_reload_probe_runner.mjs <buildDir> [true|false]");
  process.exit(2);
}
const expectedForce = expectedForceArg === "true";
...
if (reloads.length !== 1 || reloads[0] !== expectedForce) {
  console.error(
    `expected Navigation.reload to call location.reload(${expectedForce}), got ${JSON.stringify(reloads)}`
  );
  process.exit(1);
}
...
console.log("rc_ok nav_reload_ok");
