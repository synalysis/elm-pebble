...
if (!buildDir) {
  console.error("usage: wasm_bytes_endian_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
if (innerText !== "le" && innerText !== "be") {
  console.error(
    `expected Task.perform Bytes.getHostEndianness to yield le or be, got ${JSON.stringify(innerText)}`
  );
  process.exit(1);
}
...
console.log(`rc_ok host_endianness=${innerText}`);
