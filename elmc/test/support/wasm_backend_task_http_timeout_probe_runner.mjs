...
if (!buildDir) {
  console.error("usage: wasm_backend_task_http_timeout_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
if (innerText !== "timeout") {
  console.error(
    `expected Timeout error label in view, got innerText=${JSON.stringify(innerText)}`
  );
  process.exit(1);
}

if (!helpers.checkBalanced()) {
  console.error("RC imbalance after backend task http timeout boot");
  process.exit(1);
}

console.log("rc_ok backend_task_http_timeout_ok");
