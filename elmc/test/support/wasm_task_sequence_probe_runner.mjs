...
if (!buildDir) {
  console.error("usage: wasm_task_sequence_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
if (innerText !== "1,2,3;err=9") {
  console.error(
    `expected Task.sequence async + fail short-circuit, got ${JSON.stringify(innerText)}`
  );
  process.exit(1);
}
...
console.log("rc_ok task_sequence_ok");
