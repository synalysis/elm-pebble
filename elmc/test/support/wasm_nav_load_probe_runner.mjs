...
if (!buildDir) {
  console.error("usage: wasm_nav_load_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
if (assigned.length !== 1 || assigned[0] !== "https://example.com/next") {
  console.error(
    `expected Navigation.load to assign https://example.com/next, got ${JSON.stringify(assigned)}`
  );
  process.exit(1);
}
