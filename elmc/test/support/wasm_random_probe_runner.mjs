...
if (!buildDir) {
  console.error("usage: wasm_random_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
if (innerText !== "ok:7,7,7") {
  console.error(
    `expected Random.constant + list/int generators, view=${JSON.stringify(innerText)}`
  );
  process.exit(1);
}
...
if (!helpers.checkBalanced()) {
  ...
  if (state) {
    console.warn("RC imbalance after random boot (non-fatal)", state);
  }
}

console.log("rc_ok random_ok");
