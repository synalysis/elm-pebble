...
if (!buildDir) {
  console.error("usage: wasm_set_viewport_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
if (innerText !== "12,34") {
  console.error(
    `expected setViewport 12 34 then getViewport, got ${JSON.stringify(innerText)}`
  );
  process.exit(1);
}
...
console.log("rc_ok set_viewport_ok");
