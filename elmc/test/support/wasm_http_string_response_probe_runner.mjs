...
if (!buildDir) {
  console.error("usage: wasm_http_string_response_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
if (fetchCalls < 1) {
  console.error("expected Http.expectStringResponse to fetch");
  process.exit(1);
}
const expected = "200:OK:https://example.com/resp:text/plain:hello";
if (innerText !== expected) {
  console.error(
    `expected official GoodStatus metadata+body ${JSON.stringify(expected)}, got ${JSON.stringify(innerText)}`
  );
  process.exit(1);
}
...
console.log("rc_ok http_string_response_ok");
