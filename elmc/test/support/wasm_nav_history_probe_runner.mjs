...
const [buildDir, expectedDeltaArg] = process.argv.slice(2);
if (!buildDir || expectedDeltaArg === undefined) {
  console.error("usage: wasm_nav_history_probe_runner.mjs <buildDir> <expectedDelta>");
  process.exit(2);
}
const expectedDelta = Number(expectedDeltaArg);
...
if (deltas.length !== 1 || deltas[0] !== expectedDelta) {
  console.error(
    `expected Navigation history.go(${expectedDelta}), got ${JSON.stringify(deltas)}`
  );
  process.exit(1);
}
...
console.log("rc_ok nav_history_ok");
