...
if (!buildDir) {
  console.error("usage: wasm_set_title_probe_runner.mjs <buildDir>");
  process.exit(2);
}
...
if (document.title !== "hello-title") {
  console.error(`expected Browser.Dom.setTitle to set document.title, got ${JSON.stringify(document.title)}`);
  process.exit(1);
}
...
console.log("rc_ok set_title_ok");
