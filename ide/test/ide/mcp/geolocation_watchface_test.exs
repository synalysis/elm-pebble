defmodule Ide.Mcp.GeolocationWatchfaceTest do
  ...
  test "equator and prime meridian coordinates are not treated as missing" do
    slug = "geolocation-zero-#{System.unique_integer([:positive])}"

    assert {:ok, project} =
             Projects.create_project(%{
               "name" => "Geolocation Zero",
               "slug" => slug,
               "target_type" => "watchface",
               "template" => "companion-demo-geolocation"
             })
    ...
    assert watch_runtime_model["latitudeE6"] == 0
    assert watch_runtime_model["longitudeE6"] == 0
    assert watch_runtime_model["accuracyM"] == 10
  end
end
