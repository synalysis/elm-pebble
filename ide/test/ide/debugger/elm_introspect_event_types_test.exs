defmodule Ide.Debugger.ElmIntrospectEventTypesTest do
  ...
  test "from_introspect builds summary payload from introspect snapshot" do
    ...
    payload = ElmIntrospectEventPayload.from_introspect(ei, "src/ElmIntro.elm", "watch", true)
  ...
  test "reload appends debugger.elm_introspect when source is Elm" do
    slug = "elm_introspect_event_#{System.unique_integer([:positive])}"
    ...
    event = Enum.find(state.events, &(&1.type == "debugger.elm_introspect"))
    assert event
