defmodule Ide.Debugger.ProtocolDeliveryOrderTest do
  ...
  test "init runtime_cmd watch_to_companion queues AppMessage until recipient init completes" do
    alias Ide.Debugger.AppMessageQueue
    ...
    assert AppMessageQueue.pending?(next, :companion)
    refute PendingProtocolDelivery.pending(next) != []
    assert ProtocolRx.inline_protocol_deliveries(next) == []
  end

  test "init runtime_cmd watch_to_companion defers to pending protocol when recipient init is complete" do
    alias Ide.Debugger.AppMessageQueue
    alias Ide.Debugger.PendingProtocolDelivery
    ...
    refute AppMessageQueue.pending?(next, :companion)
    assert ProtocolRx.inline_protocol_deliveries(next) == []
  end
end
