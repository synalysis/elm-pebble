defmodule Ide.Emulator.SessionTest do
  ...
  test "embedded pypkjs companion cache refresh starts phone JS" do
    ...
    assert source =~ "flush_pending_inbound_appmessages"
    assert source =~ "schedule_companion_watch_resync"
    assert source =~ "schedule_companion_watch_ping"
    assert source =~ "deliver_companion_watch_ping"
    assert source =~ "deliver_watch_inbound_to_companion_js"
    assert source =~ "pebble_js_runtime"
    assert source =~ "companion_js_eval_ready"
    assert source =~ "deliverCompanionWatchInboundFromWire"
    assert source =~ "_plain_wire_value"
    assert source =~ "wire_appmessage_plain_dict"
    refute source =~ ~s/getattr(pebble.runtime, "js", None)/
    assert source =~ "Companion watch inbound delivered to Elm"
    assert source =~ "_phone_pbw_with_js"
    assert source =~ "Companion rebinding watch AppMessage uuid"
    assert source =~ "Watch companion AppMessage tag="
    assert source =~ "DEBUG_SIMULATOR_KEY_COMPANION_RESYNC"
    assert source =~ "Replayed %d inbound AppMessage(s)"
    ...
    assert source =~ "send_wire_appmessage"
    assert source =~ "Companion phone→watch AppMessage sent"
