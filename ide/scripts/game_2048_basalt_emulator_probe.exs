# Usage: cd ide && mix run scripts/game_2048_basalt_emulator_probe.exs
...
unless Emulator.runtime_status("basalt").missing == [] do
  IO.puts("embedded emulator not ready: #{inspect(Emulator.runtime_status("basalt").missing)}")
  System.halt(1)
end
...
try do
  ...
  IO.puts("game-2048 basalt fault=#{snap.fault_detected} stopped=#{stopped?}")
