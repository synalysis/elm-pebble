defmodule IdeWeb.CoreComponents do
  @moduledoc """
  ...
  def flash(assigns) do
    ...
    <div
      ...
      data-flash-key={Atom.to_string(@kind)}
      data-auto-dismiss-ms={if @kind == :info, do: "2500", else: nil}
      role="alert"
      class={[
        "fixed top-2 right-2 mr-2 w-80 sm:w-96 z-50 rounded-lg p-3 ring-1",
        @kind == :info && "bg-emerald-50 text-emerald-800 ring-emerald-500 fill-cyan-900",
        @kind == :warning && "bg-amber-50 text-amber-900 shadow-md ring-amber-500 fill-amber-900",
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(IdeWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(IdeWeb.Gettext, "errors", msg, opts)
    end
  end
