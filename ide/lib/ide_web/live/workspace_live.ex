defmodule IdeWeb.WorkspaceLive do
  ...
  def handle_info({:companion_debugger_bootstrapped, scope_key, result}, socket) do
    DebuggerFlow.handle_info({:companion_debugger_bootstrapped, scope_key, result}, socket)
  end

  def handle_info({:debugger_bootstrap_progress, token, message}, socket) do
    DebuggerFlow.handle_info({:debugger_bootstrap_progress, token, message}, socket)
  end

  def handle_info({:debugger_companion_bootstrap_progress, message}, socket)
      when is_binary(message) do
    DebuggerFlow.handle_info({:debugger_companion_bootstrap_progress, message}, socket)
  end

  def handle_info(:debugger_runtime_updated, socket) do
    DebuggerFlow.handle_info(:debugger_runtime_updated, socket)
  end

  def handle_info({:debugger_runtime_refresh, seq}, socket) when is_integer(seq) do
    DebuggerFlow.handle_info({:debugger_runtime_refresh, seq}, socket)
  end

  def handle_info({:debugger_auto_fire_refresh, project_slug}, socket) do
    DebuggerFlow.handle_info({:debugger_auto_fire_refresh, project_slug}, socket)
  end
