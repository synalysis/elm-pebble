defmodule Ide.Debugger.TriggerSurface do
  ...
  @type introspect_fn :: (Types.runtime_state(), Types.surface_target() -> Types.elm_introspect() | map())
  ...
  def candidates(state, target, %{introspect_for: introspect_for, source_root_for_target: source_root_for_target})
      when is_map(state) and target in [:watch, :companion, :phone] and is_function(introspect_for, 2) and
  ...
  def display_for(state, trigger, target_name, introspect_for, normalize_target)
      when is_map(state) and is_binary(trigger) and is_binary(target_name) and is_function(introspect_for, 2) and
  ...
  def display_for(_state, _trigger, _target_name, _introspect_for, _normalize_target), do: "Trigger"
