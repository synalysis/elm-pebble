defmodule Ide.Debugger.SessionApi do
  ...
  def start_session(project_slug) when is_binary(project_slug), do: start_session(project_slug, %{})
  ...
  def start_session(project_slug, attrs) when is_binary(project_slug) and is_map(attrs) do
    AgentSession.with_hosts(fn hosts ->
      AgentSession.mutate(project_slug, &SessionStartReset.start(&1, project_slug, attrs, hosts.lifecycle))
    end)
  end
  ...
  def reset(project_slug) when is_binary(project_slug) do
    AgentSession.with_hosts(fn hosts ->
      AgentSession.mutate(project_slug, &SessionStartReset.reset(&1, project_slug, hosts.lifecycle))
    end)
  end
  ...
  def set_watch_profile(project_slug, attrs \\ %{}) when is_binary(project_slug) and is_map(attrs) do
    AgentSession.with_hosts(fn hosts ->
  ...
  def set_simulator_settings(project_slug, attrs \\ %{}) when is_binary(project_slug) and is_map(attrs) do
    settings = SettingsApi.normalize(attrs)

    AgentSession.with_hosts(fn hosts ->
      AgentSession.mutate(project_slug, &SettingsApi.apply_to_state(&1, settings, hosts.simulator_settings))
    end)
  end
end
