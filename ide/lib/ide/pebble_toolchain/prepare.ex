defmodule Ide.PebbleToolchain.Prepare do
  ...
  defp write_emulator_build_flags(app_root, opts) do
    ...
    keep_backlight_defines =
      if Keyword.get(opts, :emulator_keep_backlight, false) do
        "#define ELMC_PEBBLE_EMULATOR_KEEP_BACKLIGHT 1\n"
      else
        ""
      end
    ...
    #{watchface_define}#{storage_log_defines}#{probe_defines}#{heap_log_defines}#{debug_log_defines}#{keep_backlight_defines}#{catalog_watch_info_c_defines()}#endif
    """
  ...
  @doc """
  Per-platform catalog WatchInfo enums for emulator/QEMU.

  QEMU has no watch-color control protocol, so `watch_info_get_color()` is
  typically `WATCH_INFO_COLOR_UNKNOWN`. The catalog constructor for the
  compiled `PBL_PLATFORM_*` is the fallback the runtime maps to Elm.
  """
  @spec catalog_watch_info_c_defines() :: String.t()
  def catalog_watch_info_c_defines do
    branches =
      Enum.flat_map(WatchModels.ordered_ids(), fn id ->
        profile = WatchModels.profile_for(id)
        color_sdk = sdk_watch_info_enum(WatchInfoMap.colors(), profile["watch_info_color"])
        model_sdk = sdk_watch_info_enum(WatchInfoMap.models(), profile["watch_info_model"])
        platform = String.upcase(id)

        if is_binary(color_sdk) and is_binary(model_sdk) do
          [
            """
            #elif defined(PBL_PLATFORM_#{platform})
            #define ELMC_PEBBLE_CATALOG_WATCH_COLOR #{color_sdk}
            #define ELMC_PEBBLE_CATALOG_WATCH_MODEL #{model_sdk}
            """
          ]
        else
          []
    ...
    case branches do
      ...
      [first | rest] ->
        [String.replace_prefix(first, "#elif", "#if") | rest]
        |> Enum.join()
        |> Kernel.<>("#endif\n")
    end
  end

  defp sdk_watch_info_enum(entries, ctor) when is_binary(ctor) do
  ...
  defp sdk_watch_info_enum(_, _), do: nil
  ...
  @spec package_author(opts()) :: String.t()
  defp package_author(opts) do
  ...
  defp stage_project_resources(workspace_root, app_root) do
    with {:ok, bitmap_entries} <- stage_bitmap_resources(workspace_root, app_root),
      ...
      {:ok,
       bitmap_entries ++
         font_entries ++ vector_entries ++ animation_entries ++ speaker_sample_entries}
    end
  end
        ...
        with {:ok, decoded} <- Jason.decode(json),
          ...
          media_entries =
            ...
            |> Enum.map(fn row ->
              ...
              tracking_adjust =
                row |> Map.get("tracking_adjust", 0) |> Coercion.integer_or_default(0)
