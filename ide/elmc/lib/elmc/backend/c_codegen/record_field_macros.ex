defmodule Elmc.Backend.CCodegen.RecordFieldMacros do
  ...
  def definitions(%IR{} = ir, opts \\ []) do
    ...
    macros = Map.new(entries, fn {key, macro, _index} -> {key, macro} end)

    {defines, macros}
  end

  @doc """
  Rebuild the field enum from an existing macro map (session ∪ late used fields).

  Indices come from full record shapes so emit-time and late defines stay aligned.
  """
  @spec defines_from_macro_map(IR.t(), macro_map()) :: String.t()
  def defines_from_macro_map(%IR{} = ir, macro_map) when is_map(macro_map) do
    shapes = IRQueries.record_alias_shape_map(ir)

    entries =
      for {{mod, type, field}, macro} <- macro_map,
          fields when is_list(fields) <- [Map.get(shapes, {mod, type})],
          index when is_integer(index) <- [Enum.find_index(fields, &(&1 == field))] do
        {macro, index, {mod, type, field}}
      end

    entries
    |> Enum.sort_by(fn {_macro, _index, key} -> key end)
    |> Enum.map(fn {macro, index, _key} -> "  #{macro} = #{index}" end)
    |> case do
      [] ->
        ""
