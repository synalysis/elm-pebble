defmodule Elmc.Backend.CCodegen.DirectRender.RecordViewPeel do
  ...
  defp site_param_peel(%{caller: caller_key, args: args} = site, param_index, decl_map, visited) do
    with arg when not is_nil(arg) <- Enum.at(args, param_index),
         {:ok, caller_decl} <- Map.fetch(decl_map, caller_key) do
      case param_var_index(arg, caller_decl) do
        {:ok, caller_param_index} ->
        ...
        :error ->
          site_arg_peel(site, param_index, decl_map)
      end
    else
      _ -> :error
    end
