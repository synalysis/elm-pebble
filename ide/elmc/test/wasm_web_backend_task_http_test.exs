defmodule Elmc.WasmWebBackendTaskHttpTest do
  ...
  @tag :wasm_execute
  test "BackendTask.Http.getJson lowers to backend_task_http_get_json import" do
    cond do
      not execution_tools_available?() ->
        :ok

      true ->
        root = Path.expand("fixtures/wasm_web_backend_task_http_project", __DIR__)
        out_dir = Path.expand("tmp/wasm_web_backend_task_http", __DIR__)
        File.rm_rf!(out_dir)
        ...
        assert wat =~ "backend_task_http_get_json"
        ...
        case run_probe(out_dir) do
          ...
          {:error, output} ->
            if probe_skipped_under_ulimit?(output) do
              ...
              flunk("backend task http probe failed:\n#{output}")
            end
        end
    end
  end

  @runner Path.expand("support/wasm_backend_task_http_probe_runner.mjs", __DIR__)
