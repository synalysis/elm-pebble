defmodule Elmc.WasmWebHttpTrackTest do
  ...
  @tag :wasm_execute
  test "Http.track lowers to dom_sub progress kind under wasm_strict" do
    cond do
      not execution_tools_available?() ->
        :ok

      true ->
        root = Path.expand("fixtures/wasm_web_http_track_project", __DIR__)
        out_dir = Path.expand("tmp/wasm_web_http_track", __DIR__)
        File.rm_rf!(out_dir)
        ...
        case run_probe(out_dir) do
          ...
          {:error, output} ->
            if probe_skipped_under_ulimit?(output) do
              ...
              flunk("http track probe failed:\n#{output}")
            end
        end
    end
  end

  @runner Path.expand("support/wasm_http_track_probe_runner.mjs", __DIR__)
