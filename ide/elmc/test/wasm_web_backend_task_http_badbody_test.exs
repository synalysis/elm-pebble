defmodule Elmc.WasmWebBackendTaskHttpBadbodyTest do
  ...
  @tag :wasm_execute
  test "BackendTask.Http JSON decode failure yields typed BadBody error in browser" do
    cond do
      not execution_tools_available?() ->
        :ok

      true ->
        root = Path.expand("fixtures/wasm_web_backend_task_http_badbody_project", __DIR__)
        out_dir = Path.expand("tmp/wasm_web_backend_task_http_badbody", __DIR__)
        File.rm_rf!(out_dir)
        ...
        assert manifest["constructor_tags"]["BackendTask.Http.BadBody"] != nil
        ...
        case run_probe(out_dir) do
          ...
          {:error, output} ->
            if probe_skipped_under_ulimit?(output) do
              ...
              flunk("backend task http badbody probe failed:\n#{output}")
            end
        end
    end
  end

  @runner Path.expand("support/wasm_backend_task_http_badbody_probe_runner.mjs", __DIR__)
