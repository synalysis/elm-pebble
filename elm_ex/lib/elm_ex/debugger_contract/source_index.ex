defmodule ElmEx.DebuggerContract.SourceIndex do
  ...
  @spec api_metadata(Module.t(), [Types.import_entry()], keyword()) :: Types.source_api_metadata()
  def api_metadata(%Module{} = mod, import_entries, opts \\ []) when is_list(import_entries) do
  ...
  @spec source_roots_for_module(Module.t(), keyword()) :: [String.t()]
  defp source_roots_for_module(%Module{path: path}, opts) when is_binary(path) do
    current_roots =
      path
      |> Path.expand()
      |> Path.dirname()
    ...
    package_roots = Application.get_env(:elm_ex, :package_source_roots, []) ++ extra
