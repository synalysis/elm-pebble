defmodule ElmEx.DebuggerContract do
  @moduledoc """
  Static Elm AST analysis for debugger compile-time metadata (`debugger_contract.v1`).

  Consumers attach output at compile ingest; the IDE reads it via
  `Ide.Debugger.CompileContract`, not by re-parsing on hot reload.
  """
  ...
  alias ElmEx.DebuggerContract.SourceIndex
  ...
  Options:
  - `:extra_source_roots` — additional directories for cross-module type resolution
  """
  @spec analyze_source(String.t(), String.t(), keyword()) ::
          {:ok, Types.introspect_snapshot()} | {:error, Types.parse_error()}
  def analyze_source(source, virtual_path \\ "Main.elm", opts \\ [])
      when is_binary(source) and is_binary(virtual_path) do
    with {:ok, %Module{} = mod} <- GeneratedParser.parse_source(virtual_path, source) do
      {:ok, build_snapshot(mod, source_display_path(virtual_path), source, opts)}
    end
  end
  ...
  def unwrap_shell(shell) when is_map(shell) do
    case Map.get(shell, "debugger_contract") || Map.get(shell, :debugger_contract) do
      ...
      _ -> Map.get(shell, "elm_introspect") || Map.get(shell, :elm_introspect) || shell
    end
  end

  def unwrap_shell(_), do: nil

  @doc """
  Extracts the inner debugger contract map from a snapshot (`analyze_source/2`, `from_project/2`, …).
  """
  @spec contract_payload(Types.introspect_snapshot() | Types.elm_introspect()) ::
          Types.elm_introspect() | nil
  def contract_payload(%{"debugger_contract" => %{} = payload}), do: payload
  def contract_payload(%{"elm_introspect" => %{} = payload}), do: payload
  def contract_payload(%{} = payload) when is_map(payload), do: payload
  def contract_payload(_), do: nil
  ...
  def from_project(%Project{} = project, opts \\ []) do
    ...
    case find_entry_module(project) do
      %Module{} = mod ->
        {:ok, build_snapshot(mod, nil, nil, opts)}
  ...
  def from_project_dir(project_dir, opts \\ []) when is_binary(project_dir) do
    ...
    opts =
      if extra != [] do
        Keyword.put(opts, :extra_source_roots, extra)
      else
        opts
      end

    with {:ok, %Project{} = project} <- Bridge.load_project(project_dir) do
      from_project(project, opts)
    end
  end
  ...
  defp find_entry_module(%Project{modules: modules, project_dir: project_dir}) do
    ...
    Enum.find(modules, fn
      %Module{name: ^entry_name} -> true
      _ -> false
    end)
  end

  @spec entry_module_name(String.t()) :: String.t()
  defp entry_module_name(project_dir) when is_binary(project_dir) do
    cond do
      File.exists?(Path.join(project_dir, "src/CompanionApp.elm")) ->
        "CompanionApp"
  ...
  defp build_snapshot(
    ...
    function_types = SourceIndex.function_type_index(mod, import_entries, opts)

    api_metadata =
      mod
      |> SourceIndex.api_metadata(import_entries, opts)
    ...
    update_ctor_model_fields =
      EffectAnalysis.update_ctor_model_fields(update_e, update_params)
  ...
  @spec snapshot_source_tag(String.t() | nil, String.t() | nil) :: String.t()
  defp snapshot_source_tag(nil, nil), do: "project_module"
  defp snapshot_source_tag(_path, _source), do: "elmc_parser"
  ...
  @spec msg_constructor_arg_types(%Module{} | nil) :: %{optional(String.t()) => String.t()}
  defp msg_constructor_arg_types(%Module{declarations: decls}) do
    decls
    |> msg_union()
    |> case do
      %{constructors: ctors} when is_list(ctors) ->
        ctors
        |> Enum.flat_map(fn
          %{name: name, arg: arg} when is_binary(name) and is_binary(arg) and arg != "" ->
            [{name, arg}]
